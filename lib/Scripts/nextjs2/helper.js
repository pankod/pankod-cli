...
exports.Helper = {
    ...
    createInterface: (answers, isClass, createInterfaceParams) => {
        ...
        if (isConnectStore) {
            ...
            setTimeout(() => {
                const replaceStoreImportParams = commonReplaceParams(createInterfaceParams.storeImportInterface, 'Interface file added to import section in Redux/IStore.d.ts', /\s[}] from "@Interfaces";/g);
    ...
    addReducer: (answers, params) => {
        ...
        const reducerFolderDir = `${config_1.Config.nextjs2.reducerDir}/${lowerFileName}`;
        ...
        const addTestParams = {
            dirPath: testFileDir,
            getFileContent: () => Common_1.CommonHelper.getTemplate(reducerTestTemplatePath, templateProps),
        ...
        Common_1.CommonHelper.createFile(reducerFolderDir);
        Common_1.CommonHelper.writeFile(writeFileProps);
        Common_1.CommonHelper.replaceContent(replaceContentParams);
        Common_1.CommonHelper.writeFile(addTestParams);
        setTimeout(() => {
    ...
    createFuncComponent: (answers, params) => {
        ...
        const writeTestFileProps = {
            ...
            getFileContent: () => Common_1.CommonHelper.getTemplate(params.componentTestTemplatePath, templateProps),
            message: 'Added unit test of component.'
        };
        Common_1.CommonHelper.createFile(funcComponentDir);
        Common_1.CommonHelper.writeFile(writeFileProps);
        Common_1.CommonHelper.writeFile(writeTestFileProps);
        Common_1.CommonHelper.addToIndex(addIndexParams);
