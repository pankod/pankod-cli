...
exports.Helper = {
    ...
    createInterface: (answers, isClass, createInterfaceParams) => {
        ...
        const replaceContentParams = {
            ...
            getFileContent: () => Common_1.CommonHelper.getTemplate(isPage
    ...
    createStyle: (answers, createStyleParams) => {
        ...
        if (isStyledComponent) {
            _pageDirPath = `${pageStyledDirPath}/${answers.fileName.replace(/\b\w/g, foo => foo.toUpperCase())}.ts` || '';
        }
        else {
    ...
    addActionConstIndex: (templateProps, params) => {
        ...
        const replaceContentParams = {
            fileDir: `${config_1.Config.nextjs2.definitionsDir}/ActionConsts.ts`,
            filetoUpdate: fs.readFileSync(path.resolve('', `${config_1.Config.nextjs2.definitionsDir}/ActionConsts.ts`), 'utf8'),
    ...
    addAction: (answers, params) => {
        ...
        const actionFileDir = `${config_1.Config.nextjs2.actionDir}/${fileName}Actions.ts`;
