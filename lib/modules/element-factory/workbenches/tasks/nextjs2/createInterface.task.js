...
exports.createInterface = (options) => {
    ...
    const commonReplaceParams = (contentFile, message, regexKey) => {
        ...
        if (params.filetoUpdate.match(formattedRegexKey)) {
            ...
            params.getFileContent = () => operations_1.getTemplate(storeImportInterfaceFormatted, options);
        }
        return params;
    };
    ...
    if (isConnectStore) {
        ...
        const replaceStoreImportParams = commonReplaceParams(storeImportInterface, 'Interface file added to import section in Redux/IStore.d.ts', / } from "@Interfaces";/);
