...
exports.createInterface = (options) => {
    ...
    const { interfaceDir, pageInterfaceIndex, pageInterfaceDir, componentsDir, compInterfaceIndex, templatePath, reduxInterfaceDir, storeImportInterface, storeImportInterfaceFormatted, storeInterface } = nextjs2_params_1.createInterfaceParams;
    ...
    const replaceContentParams = Object.assign({ fileDir: interfaceDir, filetoUpdate: fs.readFileSync(path.resolve('', interfaceDir), 'utf8'), message: 'Interface file added to Interfaces/index.ts' }, (isPage
        ? {
            getFileContent: () => operations_1.getTemplate(pageInterfaceIndex, options),
            regexKey: /\/\/(?: |)#endregion Page Interfaces/g
        }
        : {
            getFileContent: () => operations_1.getTemplate(compInterfaceIndex, options),
